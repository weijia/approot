...
class TaggingService(beanstalkWorkingThread):
    def __init__ ( self, inputTubeName = None):
        super(TaggingService, self).__init__(inputTubeName)
        self.gui_service = GuiService()
        #Register to drop service. Service will create drop window and send the dropped items to tube
        self.gui_service.addItem({"command": "DropWnd", "target": self.get_input_tube_name()})
    ...
    def processItem(self, job, item):
        ...
        self.gui_service.addItem({"command": "Browser", "url":"http://127.0.0.1:"+str(g_config_dict["ufs_web_server_port"])+"/objsys/tagging/?"+
                                    links + "encoding="+settings.DEFAULT_CHARSET, "handle": "tagging"})
        job.delete()
